...
local function _uiScrollbox(name, parent) 
	...
	--local elementColor = LibNK.art.GetThemeColor("elementSubColor2")
	...
	local scrollBox = LibNK.UICreateFrame ('nkFrame', name, parent)
	local scrollLane = LibNK.UICreateFrame ('nkFrame', name .. 'lane', scrollBox)
	local scrollLaneInner = LibNK.UICreateFrame ('nkFrame', name .. 'inner', scrollLane)
	local scrollPos = LibNK.UICreateFrame ('nkFrame', name .. 'pos', scrollLaneInner)
	...
	function scrollBox:ProcessMove()
		...
		LibNK.eventHandlers[name]["ScrollboxChanged"](newValue)
	end
	...
	LibNK.eventHandlers[name]["ScrollboxChanged"], LibNK.Events[name]["ScrollboxChanged"] = Utility.Event.Create(addonInfo.identifier, name .. "ScrollboxChanged")
