...
local function _uiMenu(name, parent) 
	...
	----if LibNK.internal.checkEvents (name, true) == false then return nil end

	local menu = LibNK.UICreateFrame ('nkCanvas', name, parent)
	...
	function menu:AddEntry (newEntry)
		local menuEntry = LibNK.UICreateFrame('nkMenuEntry', name .. '.entry.' .. (#entries + 1), menu)
		...
		if newEntry.callBack ~= nil then		
			Command.Event.Attach(LibNK.Events[name .. '.entry.' .. (#entries + 1)].Clicked, function ()
				newEntry.callBack()
	...
	function menu:AddSeparator()
		...
		local separator = LibNK.UICreateFrame ('nkFrame', name .. '.separator' .. (#entries + 1), menu)
		...
		local line = LibNK.UICreateFrame ('nkFrame', name .. '.separator' .. (#entries + 1) .. '.line', separator)
