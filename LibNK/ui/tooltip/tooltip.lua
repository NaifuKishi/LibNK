...
local function _uiTooltip(name, parent) 
	...
	local tooltipInner = LibNK.UICreateFrame ('nkFrame', name .. 'Innner', tooltip)
	local title = LibNK.UICreateFrame ('nkText', name .. 'Title', tooltipInner)
	local subTitle = LibNK.UICreateFrame ('nkText', name .. 'subTitle', tooltipInner)
	local separator = LibNK.UICreateFrame ('nkFrame', name .. 'separator', tooltipInner)
	...
	function tooltip:SetFont (addonId, fontName)
		...
		LibNK.UI.SetFont(subTitle, addonId, fontName)

		for idx = 1, #lines, 1 do
			local line = lines[idx]
			LibNK.UI.SetFont(line, addonId, fontName)
		end
	end
	...
	function tooltip:SetLines(newLines)
		...
		for idx = 1, #newLines, 1 do
			local line = nil
			if idx > #lines then
				line = LibNK.UICreateFrame ('nkText', name .. 'line' .. idx, tooltipInner)
				line:SetFontColor(defaultLinesColor[1], defaultLinesColor[2], defaultLinesColor[3], defaultLinesColor[4])
				line:SetFontSize(11)

				if font ~= nil then
					LibNK.UI.SetFont(line, font.addonId, font.fontName)
				end
	...
	function tooltip:SetVisible(flag)
		...
		if #lines == 0 then
			...
			local key, value = LibNK.Tools.Table.GetFirstElement(lines)
