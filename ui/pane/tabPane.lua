...
local function tabHeader (name, parent)
  ...
  local tabButton = LibNK.UICreateFrame("nkCanvas", name, parent)  
  local label = LibNK.UICreateFrame("nkText", name .. ".label", tabButton)
  local selectedLine = LibNK.UICreateFrame("nkFrame", name .. ".selectedLine", tabButton)
  ...
  function tabButton:SetFont(addonInfo, font) LibNK.UI.SetFont(label, addonInfo, font) end
...
local function _uiTabpane(name, parent) 
	...
	local tabPane = LibNK.UICreateFrame("nkFrame", name, parent)
	
	if tabPane == nil then return nil end -- event check failed
	
	local uiFrame = LibNK.UICreateFrame("nkCanvas", name .. ".ui", tabPane)
	local bodyFrame = LibNK.UICreateFrame("nkFrame", name .. ".body", uiFrame)
	local helperLine = LibNK.UICreateFrame("nKFrame", name .. ".helperLine", tabPane)
	...
	--local uiFrameFill = { type = 'solid', r = LibNK.art.GetThemeColor('tabPaneColor')[2].r, g = LibNK.art.GetThemeColor('tabPaneColor')[2].g, b = LibNK.art.GetThemeColor('tabPaneColor')[2].b, a = LibNK.art.GetThemeColor('tabPaneColor')[2].a}
	local uiFrameFill
	...
	local uiFrameStroke
	--local uiFrameStroke = { thickness = 1, r = LibNK.art.GetThemeColor('tabPaneColor')[1].r, g = LibNK.art.GetThemeColor('tabPaneColor')[1].g, b = LibNK.art.GetThemeColor('tabPaneColor')[1].b, a = LibNK.art.GetThemeColor('tabPaneColor')[1].a}
	
	local fontColor
	--local fontColor = LibNK.art.GetThemeColor('tabPaneColor')[4]
	...
	function tabPane:AddPane(newPaneInfo, updateFlag)
		...
		Command.Event.Attach(LibNK.Events[stepButtonName].Clicked, function ()
	...
	function tabPane:SwitchToPane(number)
		...
		LibNK.eventHandlers[name]["TabPaneChanged"](activePane)
	...
	LibNK.eventHandlers[name]["TabPaneChanged"], LibNK.Events[name]["TabPaneChanged"] = Utility.Event.Create(addonInfo.identifier, name .. "TabPaneChanged")
