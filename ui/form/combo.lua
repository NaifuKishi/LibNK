...
local function _uiCombobox(name, parent) 
	...
	local elementColor, elementColorInner, selectedColor, labelColor, borderColor

	--local elementColor = LibNK.art.GetThemeColor("elementMainColor")
	--local elementColorInner = LibNK.art.GetThemeColor("elementSubColor")
	--local selectedColor = LibNK.art.GetThemeColor("highlightColor")
	--local labelColor = LibNK.art.GetThemeColor("labelColor")
	--local borderColor = LibNK.art.GetThemeColor("elementSubColor2")
	
	local isActive = true
	--local displayWidth = 100

	local combo = LibNK.UICreateFrame ('nkFrame', name, parent)
	local comboLabel = LibNK.UICreateFrame ('nkText', name .. '.comboLabel', combo)
	
	local display = LibNK.UICreateFrame ('nkFrame', name .. '.display', combo)
	local displayInner = LibNK.UICreateFrame ('nkFrame', name .. '.displayInner', display)
	local icon = LibNK.UICreateFrame ('nkTexture', name .. '.icon', displayInner)
	local label = LibNK.UICreateFrame ('nkText', name .. '.label', displayInner)
	
	local arrowBox = LibNK.UICreateFrame ('nkFrame', name .. '.arrowBox', displayInner)
	local arrowBoxInner = LibNK.UICreateFrame ('nkFrame', name .. '.arrowBoxInner', arrowBox)
	local arrowText = LibNK.UICreateFrame ('nkText', name .. '.arrowText', arrowBoxInner)
	local selFrame = LibNK.UICreateFrame ('nkFrame', name .. '.selFrame', combo)
	local selFrameInner = LibNK.UICreateFrame ('nkFrame', name .. '.selFrameInner', selFrame)
	
	local selFrameSlider = LibNK.UICreateFrame('nkScrollbox', name .. '.selFrameSlider', selFrame)
	
	local selItems = {}	
	
	for idx = 1, 5, 1 do
		local selItemFrame = LibNK.UICreateFrame ('nkFrame', name .. 'selItemFrame' .. idx, selFrameInner)
		local selItemLabel = LibNK.UICreateFrame ('nkText', name .. 'selItemLabel' .. idx, selItemFrame)
		local selItemIcon = LibNK.UICreateFrame ('nkTexture', name .. 'selItemIcon' .. idx, selItemFrame)
		...
		selItemFrame:EventAttach(Event.UI.Input.Mouse.Left.Click, function ()
			...
			LibNK.eventHandlers[name]["ComboChanged"](selValue)
	...
	Command.Event.Attach(LibNK.Events[eventName].ScrollboxChanged, function ()
	...
	function combo:SetSelection(selection, sort)
		...
		if selection == nil or #selection == 0 then
			LibNK.Tools.Error.Display (addonInfo.identifier, stringFormat("_uiCombobox - invalid number of parameters\nexpecting: selection (table)\nreceived: %s", LibNK.Tools.Table.Serialize (selection))) 
			return 
		end
	...
	function combo:SetFont(addonInfo, font)
		LibNK.UI.SetFont(comboLabel, addonInfo, font)
		LibNK.UI.SetFont(label, addonInfo, font)		
		for idx = 1, 5, 1 do
			LibNK.UI.SetFont(selItems[idx].label, addonInfo, font)
		end
	end
	...
	LibNK.eventHandlers[name]["ComboChanged"], LibNK.Events[name]["ComboChanged"] = Utility.Event.Create(addonInfo.identifier, name .. "ComboChanged")
