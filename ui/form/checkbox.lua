...
local function _uiCheckbox(name, parent) 
	...
	local elementColor, innerColor, labelColor

	--local elementColor =  LibNK.art.GetThemeColor("elementMainColor")
	--local innerColor =  LibNK.art.GetThemeColor("elementSubColor2")
  	--local labelColor = LibNK.art.GetThemeColor("labelColor")

	local checkBox = LibNK.UICreateFrame ('nkFrame', name, parent)
	
	if checkBox == nil then return end
	
	local label = LibNK.UICreateFrame ('nkText', name .. '.label', checkBox)
	local boxOuter = LibNK.UICreateFrame ('nkFrame', name .. '.boxOuter', checkBox)
	local boxInner = LibNK.UICreateFrame ('nkFrame', name .. '.boxInner', boxOuter)
	local boxMark = LibNK.UICreateFrame ('nkFrame', name .. '.boxMark', boxInner)
	...
	function checkBox:SetChecked(flag, silent)
		...
		if silent ~= true then LibNK.eventHandlers[name]["CheckboxChanged"]( flag ) end
	end
	...
	function checkBox:SetFont(addonId, font) LibNK.UI.SetFont(label, addonId, font) end
	...
	function checkBox:SetTextFont(addonInfo, fontName) LibNK.UI.SetFont(label, addonInfo, fontName) end
	...
	function checkBox:SetRound(flag)
		...
		if flag == true then			
			...
			if roundInner == nil then
				roundOuter = LibNK.UICreateFrame ('nkCanvas', name .. '.roundOuter', checkBox)
				roundInner = LibNK.UICreateFrame ('nkCanvas', name .. '.roundInner', roundOuter)
	...
	LibNK.eventHandlers[name]["CheckboxChanged"], LibNK.Events[name]["CheckboxChanged"] = Utility.Event.Create(addonInfo.identifier, name .. "CheckboxChanged")	
