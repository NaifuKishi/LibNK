...
local function _uiProgressBar(name, parent) 
	...
	local progressBar = LibNK.UICreateFrame ('nkCanvas', name, parent)
	local inner = LibNK.UICreateFrame ('nkFrame', name ..'.inner', progressBar)	
	local label = LibNK.UICreateFrame ('nkText', name .. '.label', inner)
	...
	function progressBar:SetFont (addonId, newFont)
		LibNK.UI.SetFont(label, addonId, newFont)
	end
	
	function progressBar:SetValue(newValue)
		if newValue < range[1] or newValue > range[2] then
			LibNK.Tools.Error.Display ("LibNK", stringFormat("pogressBar [%s] value out of range %d-%d",name, range[1], range[2]), 2)
			return
		end
